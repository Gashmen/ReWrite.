...
class GlandMainDictQt:
    ...
    def set_only_gland_type(self,personal_gland: dict):
        '''оставляет только "Тип":"Кабельнный ввод", всякие гайки, шайбы, ноги удаляются
        personal_gland: '''
    ...
    def set_dict_for_calculate_gland_diam(self):
        ...
        for key_number in self.gland_main_dict_copy.copy():
            if self.gland_main_dict_copy[key_number]['Тип'] == self.gland_designation and\
                ...
                self.gland_main_dict_copy[key_number]['Тип резьбы'] == self.gland_thread and\
                self.gland_main_dict_copy[key_number]['Актуальность'] == 'Да':
                continue
            else:
                ...
                if not hasattr(self,'gland_additional_marking'):
                    ...
                    if self.gland_additional_marking == 'Т':
                        if self.thread_name_in != str():
                            if self.dict_for_choose_modification_cable[key_gland]['Внутренняя трубная резьба, размер'] == self.self.thread_value_in:
                                self.key_gland.append(key_gland)
                        elif self.thread_name_out != str():
                            if self.dict_for_choose_modification_cable[key_gland]['Внешняя трубная резьба, размер'] == self.thread_value_out:
                                self.key_gland.append(key_gland)
                    if self.gland_additional_marking == 'Т-МР':
                        if self.thread_name_in != str():
                            if self.dict_for_choose_modification_cable[key_gland]['Внутренняя трубная резьба, размер'] == self.self.thread_value_in and\
